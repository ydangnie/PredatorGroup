        ...
        <div class="wtch-info-tabs">
            ...
            <div class="wtch-tab-content" id="tab-specs">
                <table class="wtch-specs-table">
                    <tr><td>Thương hiệu</td><td>{{ $product->brand ? $product->brand->ten_thuonghieu : 'N/A' }}</td></tr>
                    <tr><td>Danh mục</td><td>{{ $product->category ? $product->category->ten_danhmuc : 'N/A' }}</td></tr>
                    <tr><td>Giới tính</td><td>{{ $product->gender == 'male' ? 'Nam' : ($product->gender == 'female' ? 'Nữ' : 'Unisex') }}</td></tr>
                    <tr><td>Mã sản phẩm</td><td>{{ $product->sku ?? '---' }}</td></tr>
                </table>
            </div>
        </div>
    ...
    <script>
        ...
        function toggleWishlist(btn) {
            ...
            if(btn.classList.contains('active')) {
                icon.classList.remove('fa-regular'); icon.classList.add('fa-solid'); icon.style.color = '#d4af37';
            } else {
                icon.classList.remove('fa-solid'); icon.classList.add('fa-regular'); icon.style.color = '#fff';
            }
        }
